...
const Waves = ({
  ...
  useEffect(() => {
    ...
    function setSize() {
      ...
      canvas.width = Math.max(0, Math.floor(boundingRef.current.width));
      canvas.height = Math.max(0, Math.floor(boundingRef.current.height));
    }
    ...
    // Start the animation loop immediately for a full-page background.
    runningRef.current = true;
    frameIdRef.current = requestAnimationFrame(tick);

    // Use Page Visibility API to pause when the tab is hidden to save CPU.
  ...
  return (
    <div
      ...
      style={{ position: 'absolute', top: 0, left: 0, margin: 0, padding: 0, width: '100%', height: '100%', overflow: 'hidden', backgroundColor, pointerEvents: 'none', ...style }}
    >
